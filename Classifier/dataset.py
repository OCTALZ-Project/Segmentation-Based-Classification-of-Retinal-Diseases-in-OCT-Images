## fusion and augmentation is applied
## for one dataset
...
# Define ShiftScaleRotate with specified parameters
shift_scale_rotate = A.Compose([
    A.ShiftScaleRotate(
    shift_limit=0.1,       # Up to 10% shift
    scale_limit=0.1,       # Up to 10% scale change
    rotate_limit=10,       # Up to ±45 degrees rotation
    interpolation=cv2.INTER_LINEAR,  # Interpolation method
    border_mode=cv2.BORDER_REFLECT_101, # Border handling
    p=1.0                  # Always apply the augmentation
    ),
    A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ToTensorV2()
])
...
class ClassifierDataset(Dataset):
    def __init__(self, data, path, weight_path, segmentation, mode):
        self.segmentation = segmentation
        self.data = self.segmentation.data
        self.bscans, self.patIds, self.labels, self.num_sli = self.extract_data(path)
        self.rawsegresultsB, self.masksB= segmentation.inference(weight_path, self.bscans)
        ...
        # Augment the dataset
        self.augmented_images, self.augmented_labels, self.augmentedIds = self.augment_dataset()
    ...
    def extract_data(self, csv_path):
        ...
        bscans = self.data['bscan']
        ids = self.data['id']
        num_sli = self.data['slicen']
        #masks = self.data['mask']
        ...
        return bscans, patIds, diseases, num_sli
    """
    def fusion(self, masks):
        ...
        for ind, vals in enumerate([[2], [0, 3], [1, 4], [5]]):
            mask = np.zeros_like(self.masks, dtype=np.uint8)
            for val in vals:
                mask[self.masks == val] = 1
            if ind != 3:
                ...
                for i in range(fusion_masks.shape[0]):  # iterate over the batch
                    for c in range(fusion_masks.shape[3]):
                        fusion_masks[i, :, :, :][mask[i, :, :] == 1] = 255  # Set to white
    ...
    def augment_dataset(self):
        ...
        if self.mode == 'train':
            np.savez('/ari/users/oeren/SegClass/Training/RecData', rec=augmented_imagesB, mask=self.masksB, img=self.bscans)
        return augmented_imagesB, augmented_labels, augmentedIds
